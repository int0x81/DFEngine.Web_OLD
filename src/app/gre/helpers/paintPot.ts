export class PaintPot {

    static toHex(r: number, g: number, b: number): number {

        let rgbString = new RGB(r, g, b).toHex().toString().substr(1);
        return parseInt(rgbString, 16);
    }

    static toHexString(r: number, g: number, b: number): string {
        return new RGB(r, g, b).toHex().toString();
    }
}

class HEX {

    private hex: string | null = "#000000";
